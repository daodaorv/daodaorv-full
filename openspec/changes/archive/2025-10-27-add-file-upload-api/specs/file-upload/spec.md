...
- Region: oss-cn-beijing (北京)
- 存储类型：标准存储（Standard）
- 访问权限：公开读（Public Read）
- CDN 加速：启用
...
| 错误码 | 错误信息         | 说明                   |
| ------ | ---------------- | ---------------------- |
| 40001  | 文件格式不支持   | 文件格式不在白名单中   |
| 40002  | 文件大小超过限制 | 图片>5MB 或文档>10MB   |
| 40003  | 文件上传失败     | OSS 上传错误           |
| 40004  | 文件不存在       | 查询或删除的文件不存在 |
| 40005  | 权限不足         | 无权访问或删除文件     |
| 40006  | 批量上传失败     | 部分或全部文件上传失败 |
