# Proposal: 小程序端全局导航与路由系统

**Change ID**: `add-miniprogram-global-navigation`
**Status**: 🟡 In Progress
**Created**: 2025-10-28
**Author**: AI Assistant
**Priority**: P0 (核心功能)

---

## Why

### 业务价值

小程序端全局导航与路由系统是整个小程序应用的基础设施,必须在所有业务功能开发之前完成。该系统将:

1. **统一用户体验**: 提供一致的导航体验,用户可以快速在不同功能模块之间切换
2. **权限控制**: 实现登录拦截,保障业务流程的合规性(实名/驾照认证保留用于未来扩展)
3. **提升开发效率**: 封装路由跳转方法,简化后续业务功能的开发
4. **支持多端部署**: 兼容微信小程序、支付宝小程序、抖音小程序、H5 等多个平台
5. **符合业务逻辑**: 用户登录后即可访问订单页,在页面内填写资料即可下单,实际核验在线下进行

### 问题陈述

当前小程序端项目已完成基础环境配置,但缺少:

- TabBar 导航的实际实现
- 路由拦截器(登录、认证等)
- 统一的路由跳转封装
- 页面权限管理机制

### 目标

实现一个完整的全局导航与路由系统,包括:

1. TabBar 导航(首页/社区/客服/我的)
2. 路由拦截器(登录拦截,实名/驾照认证保留用于未来扩展)
3. 路由跳转封装(navigateTo, redirectTo, switchTab 等)
4. 页面权限管理(白名单、登录要求等)
5. 符合实际业务逻辑(线上提交资料 ≠ 线上认证,门店核验 ≠ 系统认证)

---

## What

### 功能范围

#### 1. TabBar 导航

- **首页**: 展示推荐车辆、营地、旅游路线等
- **社区**: 房车生活分享、攻略、活动等
- **客服**: 在线客服、常见问题、工单等
- **我的**: 个人中心、订单、钱包、设置等

#### 2. 路由拦截器

- **登录拦截**: 未登录用户访问需要登录的页面时,跳转到登录页
- **实名认证拦截**: 保留用于未来扩展(如提现等金融操作)
- **驾照认证拦截**: 保留用于未来扩展
- **业务逻辑**: 当前业务中,用户登录后即可访问订单页,在页面内填写资料即可下单

#### 3. 路由跳转封装

- `navigateTo`: 保留当前页面,跳转到应用内的某个页面
- `redirectTo`: 关闭当前页面,跳转到应用内的某个页面
- `switchTab`: 跳转到 tabBar 页面,并关闭其他所有非 tabBar 页面
- `reLaunch`: 关闭所有页面,打开到应用内的某个页面
- `navigateBack`: 关闭当前页面,返回上一页面或多级页面

#### 4. 页面权限管理

- 白名单页面(无需登录): 首页、社区、登录、注册等
- 需要登录的页面: 我的、订单、钱包、订单确认、支付等
- 需要实名认证的页面: 提现等金融操作(保留用于未来扩展)
- 需要驾照认证的页面: 保留用于未来扩展

### 技术实现

#### 1. TabBar 配置

在 `pages.json` 中配置 TabBar:

```json
{
  "tabBar": {
    "color": "#999999",
    "selectedColor": "#FF6B00",
    "backgroundColor": "#FFFFFF",
    "borderStyle": "black",
    "list": [
      {
        "pagePath": "pages/index/index",
        "text": "首页",
        "iconPath": "static/tabbar/home.png",
        "selectedIconPath": "static/tabbar/home-active.png"
      },
      {
        "pagePath": "pages/community/index",
        "text": "社区",
        "iconPath": "static/tabbar/community.png",
        "selectedIconPath": "static/tabbar/community-active.png"
      },
      {
        "pagePath": "pages/customer-service/index",
        "text": "客服",
        "iconPath": "static/tabbar/service.png",
        "selectedIconPath": "static/tabbar/service-active.png"
      },
      {
        "pagePath": "pages/profile/index",
        "text": "我的",
        "iconPath": "static/tabbar/profile.png",
        "selectedIconPath": "static/tabbar/profile-active.png"
      }
    ]
  }
}
```

#### 2. 路由拦截器实现

在 `utils/router.ts` 中实现路由拦截器:

```typescript
// 路由拦截器
export function routerInterceptor(url: string, options?: any) {
  const userStore = useUserStore();
  const pageConfig = getPageConfig(url);

  // 检查登录状态
  if (pageConfig.requireLogin && !userStore.isLoggedIn) {
    uni.navigateTo({ url: "/pages/auth/login" });
    return false;
  }

  // 检查实名认证
  if (pageConfig.requireRealName && !userStore.isRealNameVerified) {
    uni.navigateTo({ url: "/pages/auth/real-name" });
    return false;
  }

  // 检查驾照认证
  if (pageConfig.requireDrivingLicense && !userStore.isDrivingLicenseVerified) {
    uni.navigateTo({ url: "/pages/auth/driving-license" });
    return false;
  }

  return true;
}
```

#### 3. 路由跳转封装

在 `utils/router.ts` 中封装路由跳转方法:

```typescript
export const router = {
  navigateTo(url: string, options?: any) {
    if (routerInterceptor(url, options)) {
      uni.navigateTo({ url, ...options });
    }
  },

  redirectTo(url: string, options?: any) {
    if (routerInterceptor(url, options)) {
      uni.redirectTo({ url, ...options });
    }
  },

  switchTab(url: string, options?: any) {
    uni.switchTab({ url, ...options });
  },

  reLaunch(url: string, options?: any) {
    uni.reLaunch({ url, ...options });
  },

  navigateBack(delta: number = 1) {
    uni.navigateBack({ delta });
  },
};
```

#### 4. 页面权限配置

在 `config/page-auth.ts` 中配置页面权限:

```typescript
export const pageAuthConfig = {
  // 白名单页面(无需登录)
  whitelist: [
    "/pages/index/index",
    "/pages/community/index",
    "/pages/auth/login",
    "/pages/auth/register",
  ],

  // 需要登录的页面
  requireLogin: [
    "/pages/profile/index",
    "/pages/order/list",
    "/pages/wallet/index",
  ],

  // 需要实名认证的页面
  requireRealName: ["/pages/order/confirm", "/pages/payment/index"],

  // 需要驾照认证的页面
  requireDrivingLicense: [
    "/pages/order/confirm", // 房车租赁订单确认
  ],
};
```

### 依赖关系

- **前置依赖**:

  - ✅ uni-app 环境配置完成
  - ✅ HBuilderX 可正常编译
  - ✅ TabBar 图标已准备
  - ✅ Pinia 状态管理已配置
  - ✅ 后端用户认证 API 已完成

- **后续依赖**:
  - 所有业务功能模块都依赖本系统

### 影响范围

- **新增文件**:

  - `utils/router.ts` - 路由封装与拦截器
  - `config/page-auth.ts` - 页面权限配置
  - `stores/user.ts` - 用户状态管理(已存在,需完善)

- **修改文件**:
  - `pages.json` - TabBar 配置
  - `main.ts` - 注册路由拦截器

---

## How

### 实施计划

#### 阶段 1: TabBar 配置与页面创建 (1 天)

1. 配置 `pages.json` 中的 TabBar
2. 确认 TabBar 图标路径正确
3. 创建 4 个 TabBar 页面的基础结构
4. 测试 TabBar 切换功能

#### 阶段 2: 路由拦截器实现 (1 天)

1. 实现 `utils/router.ts` 路由封装
2. 实现路由拦截器逻辑
3. 配置 `config/page-auth.ts` 页面权限
4. 在 `main.ts` 中注册路由拦截器

#### 阶段 3: 用户状态管理完善 (1 天)

1. 完善 `stores/user.ts` 用户状态
2. 实现登录状态持久化
3. 实现认证状态获取
4. 实现 token 自动刷新

#### 阶段 4: 测试与优化 (1 天)

1. 测试 TabBar 导航功能
2. 测试路由拦截器(登录、实名、驾照)
3. 测试多端兼容性(微信、H5)
4. 性能优化与代码审查

### 验收标准

- [ ] TabBar 导航正常显示,图标正确,可以切换
- [ ] 未登录用户访问需要登录的页面时,跳转到登录页
- [ ] 未实名认证用户访问需要实名的页面时,跳转到实名认证页
- [ ] 未驾照认证用户访问需要驾照的页面时,跳转到驾照认证页
- [ ] 路由跳转封装方法可以正常使用
- [ ] 微信小程序、H5 平台测试通过
- [ ] 代码符合 ESLint 规范
- [ ] 无 TypeScript 类型错误

### 风险与缓解措施

#### 风险 1: uni-app 多端路由差异

- **影响**: 不同平台的路由行为可能不一致
- **缓解**: 使用条件编译处理平台差异,在多个平台测试

#### 风险 2: 路由拦截器性能问题

- **影响**: 每次路由跳转都需要检查权限,可能影响性能
- **缓解**: 优化拦截器逻辑,使用缓存减少重复计算

#### 风险 3: 认证状态同步问题

- **影响**: 用户认证状态可能不同步,导致拦截器判断错误
- **缓解**: 实现认证状态自动刷新机制,定期同步用户信息

---

## Success Metrics

### 功能指标

- TabBar 导航切换成功率 = 100%
- 路由拦截器准确率 = 100%
- 多端兼容性测试通过率 = 100%

### 性能指标

- 路由跳转响应时间 < 100ms
- 拦截器执行时间 < 50ms

### 质量指标

- 代码测试覆盖率 ≥ 80%
- TypeScript 类型覆盖率 = 100%
- ESLint 检查通过率 = 100%

---

## References

- [uni-app 路由与页面跳转](https://uniapp.dcloud.net.cn/api/router.html)
- [uni-app TabBar 配置](https://uniapp.dcloud.net.cn/collocation/pages.html#tabbar)
- [Pinia 状态管理](https://pinia.vuejs.org/)
- `docs/小程序端功能详细设计-优化版.md` - 5.1 全局导航(TabBar)
- `docs/技术规范手册.md` - 技术栈规范
