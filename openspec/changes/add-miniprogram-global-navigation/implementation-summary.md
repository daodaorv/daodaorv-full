...
enum PageAuthType {
  ...
  REAL_NAME_REQUIRED = 'real_name_required',  // 需要实名认证
  DRIVER_LICENSE_REQUIRED = 'driver_license_required' // 需要驾照认证
}
```
...
- 白名单: 首页、社区页、登录页
- 需要登录: 我的页、客服页
- 需要实名认证: 订单页、支付页、钱包页、众筹页
- 需要驾照认证: 房车租赁订单确认页、特惠租车订单确认页
...
function routerInterceptor(url: string): InterceptResult {
  ...
  if (isLoginRequired(path) && !isLogin) {
    return { allow: false, reason: '请先登录', redirectPath: '/pages/login/index' }
  }
  
  // 3. 检查实名认证状态
  if (isRealNameRequired(path) && !isRealNameVerified) {
    return { allow: false, reason: '请先完成实名认证', redirectPath: '/pages/auth/real-name' }
  }
  
  // 4. 检查驾照认证状态
  if (isDriverLicenseRequired(path) && !isDriverLicenseVerified) {
    return { allow: false, reason: '请先完成驾照认证', redirectPath: '/pages/auth/driver-license' }
  }
